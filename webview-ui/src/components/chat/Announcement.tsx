...
const DiscordLink = () => (
	<VSCodeLink
		href="https://discord.gg/zTkPmqjeWZ"
		onClick={(e) => {
			e.preventDefault()
			window.postMessage(
				{ type: "action", action: "openExternal", data: { url: "https://discord.gg/zTkPmqjeWZ" } },
				"*",
			)
		}}>
		Discord
...
const RedditLink = () => (
	<VSCodeLink
		...
		onClick={(e) => {
			e.preventDefault()
			window.postMessage(
				{ type: "action", action: "openExternal", data: { url: "https://x.com/MakeHubAI" } },
				"*",
			)
		}}>
		X (Twitter)
